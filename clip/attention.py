...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, layer_index, device=None, dtype=None, prefix_pool_size=0, prefix_len=0) -> None:
        ...
        if prefix_pool_size > 0:
            ...
            if layer_index < 12:
                ...
                dpw_adapter = Adapter(in_dim=embed_dim, out_dim=embed_dim, bottleneck=64, adapter_scalar=None, lr=0.05, dropout=0.1, in_proj_weight=self.in_proj_weight)
    ...
    def init_attn(self, fine_grained, tgt_task_id):
        if self.add_prefix:
            with torch.no_grad():                
                ...
                A = torch.matmul(q_matrix, k_matrix.t()).to(torch.float32)
                U, S, V = torch.svd(A)
                A_matrix = U[:, :self.prefix_len*num_heads]
                A_matrix = A_matrix.view(self.embed_dim, num_heads, self.prefix_len).permute(1, 0, 2).to(torch.float16)
                self.dpw_attn.append(A_matrix)          # num_heads, embed_dim, prefix_len
    ...
    def forward(
        ...
        if self.add_prefix:
            ...
            patch = query.transpose(0, 1).contiguous().view(patch_len*bsz, dim).clone()   # [bs*patch_len, dim]
            custom_score = torch.matmul(patch, attn).view(prefix_head, bsz, patch_len, self.prefix_len).transpose(0, 1).contiguous()
            ...
            if self.add_adapter:
                ...
                custom_value = dpw_adapter(patch).contiguous()   # [bsz, patch_len, embed_dim] 

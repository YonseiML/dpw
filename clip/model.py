...
class ResidualAttentionBlock(nn.Module):
        ...
        if add_prompt:
            if text_layer:
                self.attn = MultiheadAttention(d_model, n_head, prefix_pool_size=design_details["pool_size"], 
                                               prefix_len=design_details["language_ctx"], layer_index=i)
            else:
                self.attn = MultiheadAttention(d_model, n_head, prefix_pool_size=design_details["pool_size"], 
                                               prefix_len=design_details["vision_ctx"], layer_index=i)
        else:
            self.attn = MultiheadAttention(d_model, n_head, layer_index=i)
    ...
    def init_attn(self, fine_grained, tgt_task_id):
        self.attn.init_attn(fine_grained, tgt_task_id)

    def attention(self, x: torch.Tensor, aux_outputs, prompt_ids=None, batch_weight=None):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.attn(x, x, x, aux_outputs=aux_outputs, need_weights=False, attn_mask=self.attn_mask, prompt_ids=prompt_ids, batch_weight=batch_weight)

    def forward(self, x: torch.Tensor, aux_outputs, prompt_ids=None, batch_weight=None):
        attn_output, aux_outputs = self.attention(self.ln_1(x), aux_outputs, prompt_ids, batch_weight)
        x = x + attn_output
...
class Transformer(nn.Module):
    ...
    def init_attn(self, fine_grained, tgt_task_id):  
        for module in self.resblocks._modules.values():
            module.init_attn(fine_grained, tgt_task_id)
    

    def forward(self, x: torch.Tensor, prompt_ids=None, batch_weight=None, aux_outputs=None):
        for module in self.resblocks._modules.values():
            x, aux_outputs = module(x, aux_outputs, prompt_ids, batch_weight)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt_ids=None, batch_weight=None, aux_outputs=None):
        ...
        x, aux_outputs = self.transformer(x, prompt_ids, batch_weight, aux_outputs=aux_outputs)
